...
if importlib.util.find_spec("contok") is None:
    pytest.skip("contok module not found. Skipping C++ implementation tests.", allow_module_level=True)

# Import directly from the C++ implementation
from contok.lz import LZCoder, HierarchicalLZCoder, ensure_list, EMPTY_TOKEN
...
def test_hierarchical_basic_encode():
    ...
    coder = HierarchicalLZCoder(output_vocab_size=256)
    test_str = "hello world"
...
def test_hierarchical_context_learning():
    ...
    coder = HierarchicalLZCoder(output_vocab_size=256, input_vocab=set(range(256)))
    test_str = "hello world"
...
def test_hierarchical_encode_repeated_compression():
    ...
    second_coder = HierarchicalLZCoder(output_vocab_size=2*len(encoded_vocab), input_vocab=encoded_vocab)
    ...
    double_vocab_size = HierarchicalLZCoder(output_vocab_size=2*len(encoded_vocab), input_vocab=input_vocab)
